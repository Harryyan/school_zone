...
interface FullscreenZoneMapProps {
  school: School;
  isOpen: boolean;
  onClose: () => void;
}
...
export default function FullscreenZoneMap({ school, isOpen, onClose }: FullscreenZoneMapProps) {
  ...
  // Point-in-polygon check (simplified for demo)
  const isPointInZone = (lng: number, lat: number): boolean => {
  ...
  const geocodeAddress = async (address: string): Promise<SearchResult | null> => {
    ...
    // Mock geocoding result - in production, use actual geocoding API
    const mockCoordinates: [number, number] = [
      174.7762 + (Math.random() - 0.5) * 0.05,
      -36.8735 + (Math.random() - 0.5) * 0.05
    ];
    
    const inZone = isPointInZone(mockCoordinates[0], mockCoordinates[1]);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (result) {
      ...
      markerElement.innerHTML = `
        <div style="
          background: ${result.inZone ? '#10b981' : '#ef4444'};
      ...
      const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
        <div class="p-3">
          <h3 class="font-bold text-lg mb-1">${result.address}</h3>
          <p class="text-sm mb-2 ${result.inZone ? 'text-green-600' : 'text-red-600'}">
            ${result.inZone ? '✅ Inside school zone' : '❌ Outside school zone'}
          </p>
        </div>
      `);
    ...
    map.current.on('load', () => {
      ...
      if (map.current) {
        ...
        if (school.location) {
          ...
          const schoolMarker = new mapboxgl.Marker(schoolMarkerElement)
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg w-full h-full max-w-7xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between bg-white rounded-t-lg">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{school.name} - Enrolment Zone</h2>
            <p className="text-gray-600 mt-1">Interactive map with address zone checking</p>
          </div>
          <button
            onClick={onClose}
          ...
          <form onSubmit={handleSearch} className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Enter an address to check if it's in the school zone..."
            ...
            >
              {isSearching ? 'Searching...' : 'Check Zone'}
            </button>
          </form>

          {/* Search Result */}
          {searchResult && (
            <div className={`mt-3 p-3 rounded-lg ${searchResult.inZone ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border`}>
              <div className="flex items-center">
                <MapPinIcon className={`h-5 w-5 mr-2 ${searchResult.inZone ? 'text-green-600' : 'text-red-600'}`} />
                <div>
                  <p className="font-medium text-gray-900">{searchResult.address}</p>
                  <p className={`text-sm ${searchResult.inZone ? 'text-green-600' : 'text-red-600'}`}>
                    {searchResult.inZone 
                      ? `✅ This address is inside ${school.name}'s zone` 
                      : `❌ This address is outside ${school.name}'s zone`
                    }
                  </p>
                </div>
              </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-lg p-3">
            ...
            <div className="space-y-1 text-xs">
              ...
              <div className="flex items-center">
                <div className="w-3 h-3 bg-green-600 rounded-full border-2 border-white mr-2"></div>
                <span>In Zone</span>
              </div>
              <div className="flex items-center">
                <div className="w-3 h-3 bg-red-600 rounded-full border-2 border-white mr-2"></div>
          ...
          {/* Instructions */}
          <div className="absolute top-4 right-4 bg-white bg-opacity-90 rounded-lg p-3 text-sm text-gray-600">
            <div>💡 <strong>Tip:</strong> Search for an address above to check if it's in the school zone</div>
          </div>
        </div>
      </div>
    </div>
